  ...
  Pausable
{
  ...
  event Toss(address indexed from, address indexed to, uint256 indexed tokenId);
  ...
  modifier senderNotTimeoutedOrOnCooldown() {
    ...
    require(
      _cooldownStart[msg.sender] + COOLDOWN_BLOCK_LENGTH <= block.number ||
    ...
    returns (Snowball[] memory)
  {
    ...
    for (uint256 index; index < amount; index++) {
      snowballs[index] = _snowballs[round][getTokenOwner(round, addr, index)];
    }
  ...
  function _levelup(address to, uint256 tokenId) internal {
    uint256 roundId = getRoundId();
    uint256 amountOldPartners = _snowballs[roundId][tokenId].partners.length;
